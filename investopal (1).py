...
def create_advanced_chart(stock_data, metrics, ticker):
    ...
    fig.add_trace(go.Scatter(x=stock_data.index, y=stock_data["Adj Close"], name='Price'), row=1, col=1)
    ...
    if stock_data is not None:
        ...
        if metrics:
            ...
            for a in generate_ai_advice(ticker, risk_category, metrics, selected_risk):
                st.write(a)
            st.plotly_chart(create_advanced_chart(stock_data, metrics, ticker))
