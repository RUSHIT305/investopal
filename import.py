    ...
    if stock_data is not None:
        ...
        if metrics:
            ...
            if risk_category == selected_risk and metrics['sharpe_ratio'] > 1:
                ...
                st.markdown('<div class="badge red">⚠️ Risky</div>', unsafe_allow_html=True)

            # 4. News Headlines
            st.subheader("📰 Latest News")
            news_list = get_latest_news(ticker)
            if news_list:
                for n in news_list:
                    st.markdown(f"[{n['title']}]({n['link']}) — *{n.get('publisher', 'Unknown')}*")
            else:
